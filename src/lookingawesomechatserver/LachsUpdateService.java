...
public class LachsUpdateService{
	...
	public static final void main(String... aArgs) throws IOException{
		...
		new LachsUpdateService();
	}
	
	public LachsUpdateService()
	{
		this.enigma = Enigma.getConsole();
		this.u = new Utils(enigma,"english");
		
		printWelcome();
		
		this.host = u.getLocalIP("LAChS Update Service", true);
		this.port = 32123;
		this.isServer = true;
		
		currentPath = new File(".Downloads");
		
		init();
		
	}
	public LachsUpdateService(String host, int port, Utils u, boolean isServer)
	{
		if(host == null) host = u.getLocalIP("LachsUpdateService", true);
	...
	public Thread LPBServiceServer = new Thread(){
	...
	public Thread LPBServiceClientListener = new Thread(){
